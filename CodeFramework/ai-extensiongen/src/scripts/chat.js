...
class ChatUI {
    ...
    // =============
    async sendMessage() {
        // 1. Get checked checkboxes for javaGenerationMode
        const checkedModes = Array.from(document.querySelectorAll('input[name="javaGenerationMode"]:checked'));
        if (checkedModes.length > 1) {
            // Multiple modes checked: run all in parallel and display as they finish
            ...
            // For each checked mode, generate code in parallel
            checkedModes.forEach(async (checkbox) => {
                try {
                    const mode = checkbox.value;
                    // Set codeGeneratorType based on mode
                    let codeGeneratorType = '';
                    if (mode === 'PAGE') codeGeneratorType = 'SELENIUM_JAVA_PAGE_ONLY';
                    else if (mode === 'TEST') codeGeneratorType = 'SELENIUM_JAVA_TEST_ONLY';
                    else if (mode === 'FEATURE FILE') codeGeneratorType = 'CUCUMBER_ONLY';
                    else codeGeneratorType = this.codeGeneratorType;
                    ...
                    const finalPrompt = getPrompt(codeGeneratorType, {
                    ...
                    // Show result as soon as it's ready
                    this.addMessageWithMetadata(
                        `<b>${mode}:</b>\n\n${messageContent}`,
                        'assistant',
                        { inputTokens, outputTokens }
                    );
                } catch (err) {
                    this.addMessage(`Error generating for ${checkbox.value}: ${err.message}`, 'system');
                }
            });

            // Re-enable UI after all are done (optional: you can track with Promise.all if you want)
            setTimeout(() => {
            ...
            // Reset inspector and DOM content
            this.selectedDomContent = null;
            this.inspectorButton.classList.remove('has-content','active');
            this.inspectorButton.innerHTML = `
                <i class="fas fa-mouse-pointer"></i>
        ...
        // Fallback: single mode (original logic)
        const userMsg = this.inputField.value.trim();
        let apiRef = null;
